...
//export let isTestingMode = true;
export let isTestingMode = false;
// Create axios client, pre-configured with baseURL
const APIKit = axios.create({
	//baseURL: isTestingMode ? 'https://testing.zabehaty.uae.zabe7ti.website/' : 'https://zabehaty.uae.zabe7ti.website/',
	timeout: 160000,
...
export const changeURL = async (country, appType = 'ZABEHA') => {
	let testAdminUrl = 'https://test.halalmazarana.com/public/';
	...
	switch (country) {
		case 2:
			liveUrl = isTestingMode ? 'https://test.halalmazarana.com/public/' : 'https://sa.halalmazarana.com/';

			break;
		default:
			liveUrl = isTestingMode ? 'https://test.halalmazarana.com/public/' : 'https://new.halalemarat.com/back/public/';

			break;
	}
	APIKit.defaults.baseURL = liveUrl;

	console.log(liveUrl);

	let base_url = isTestingMode ? testAPIUrl : liveUrl; //liveUrl;
	//alert(base_url);
	try {
...
APIKit.interceptors.request.use(async function (config) {
	...
	config.baseURL = url ? url : 'https://new.halalemarat.com/back/public/';
	//config.baseURL = 'https://testing.zabehaty.uae.zabe7ti.website/';
	if (token != null) {
